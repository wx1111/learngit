...
tf.app.flags.DEFINE_string('subset', 'train', '''数据用途，训练或验证''')

tf.app.flags.DEFINE_integer('num_shard', 5, '''期待生成tfrecord文件个数''')
tf.app.flags.DEFINE_integer('num_thread', 5, '''数据预处理进程数''')
# 一个进程最少处理一个shard

tf.app.flags.DEFINE_string('output_dir', '/mnt/shared/easydl/train_data_dir_tfreocrd', '''输出路径''')
...
def find_image_files(data_dir):
    ...
    # 文件中一共有n个文件夹，所有一共是n个类
    for i in tf.gfile.ListDirectory(data_dir):
        print(i)
        print(data_dir + i)
        filename_one_label = []
        try:
          for j in list(['*.png','*.jpg','*.bmp','*.jpeg']):
            print(j)
...
def process_image(filename):
    ...
    import PIL.Image
    print(type(PIL.Image.open(filename)))
...
def convert_to_example(filename, image_buffer, label, human, height, width):
    colorspace = 'RGB'
    channels = 3
    image_format = 'JPEG'
...
def process_image_files_batch(thread_index, ranges, name, filenames, labels, humans, num_shards):
    global num_example, num_corrupted
    num_threads = len(ranges)
    num_shards_per_batch = int(num_shards / num_threads)
    # batch==thread???
    # num_shards_per_batch 必须大于1
        ...
        for i in files_in_shard:
            ...
            try:
                ...
                try:
                    ...
                    lock.release()
                continue
            example = convert_to_example(filename, image_buffer, label, human, height, width)
...
def process_image_files(filenames, human, labels):
    ...
    for thread_index in range(len(ranges)):
        args = (thread_index, ranges, FLAGS.subset, filenames, labels, human, num_shards)
...
def main(unused_argv):
    ...
    #    assert os.listdir(FLAGS.train_data_dir), '训练样本不存在'
    #    assert os.listdir(FLAGS.eval_data_dir), '验证样本不存在'

    if not tf.gfile.Exists(FLAGS.output_dir):
        tf.gfile.MakeDirs(FLAGS.output_dir)
    #    filenames, labels, human, labels_dictionary = find_image_files(FLAGS.data_dir)
    filenames, labels, human = find_image_files(FLAGS.data_dir)
    num_examples = {}
    ...
    process_image_files(filenames, human, labels)
