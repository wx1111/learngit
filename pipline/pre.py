...
def parse_example_proto(example_serialized):
  ...
  return features['image/encoded'], label, features['image/class/text'], features['image/filename']

def preprocess(image_buffer, is_training, model_name, height, width):
  
  image = tf.image.decode_jpeg(image_buffer, channels=3)#, dtype=tf.float32)
  
  pred = tf.equal(is_training,'train')
  #pred = tf.Print(pred,[model_name])
